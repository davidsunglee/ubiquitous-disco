 ...
 *
 * Scoring credits the OPPOSING Team, so a goal at one Bell increments the other
 ...
 * predicted sim events) means it fires exactly once per real goal on every
 ...
 * decrement). A missing entry is treated as 0 so the first goal still registers.
 */
